...
export default function Gallery() {
  ...
  useEffect(() => {
    rowRefs.forEach((ref, idx) => {
      ...
      const direction = idx === 1 ? 1 : -1; // middle row goes right, others go left
      const distance = ref.current.scrollWidth / 2;

      gsap.to(ref.current.children, {
        x: direction * -distance, 
        ease: "none",  // super smooth linear movement
        repeat: -1,
        duration: 30 + idx * 5, // adjust speed per row
        modifiers: {
          x: (x) => `${(parseFloat(x) % distance)}px`, // seamless loop
        },
      });
    });
  }, []);
